...
static void cursor_position_callback(GLFWwindow *window, double xpos, double ypos)
{
    if (drag)
    {
        // We add the difference between the current mouse position and the
        // previously recorded mouse position.
...
// Handle zooming
/*
        This method works by calculating the domain and range of the graph, then
   adding both ends to the current center. For example, say we have the ends of
   -10 and 10 and the new domain is 8. We first find the middle of the ends,
   which is 0, then evenly distribute the domain to both parts. So our new ends
   are -4 and 4;
*/
static void scroll_callback(GLFWwindow *window, double xoffset, double yoffset)
{
